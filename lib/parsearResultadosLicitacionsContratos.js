...
const { createDB, insertDB, existsRecord, getAllIDsfromEntityBetweenDates } = require('./sqliteAccions');
...
async function parsearResultadosLicitacionsContratos(browser, page, table, concello) {
    ...
    // en lugar de recolher os cambios cando se fai o insertDB porque non existe "record" (existsRecord = false), facemolo aqui cun resumo, comprobando
    ...
    // axeitados
    console.log("Enviando informe de cambios")
    
    const today = new Date().toISOString().slice(0, 10);
    const tenDaysMore = new Date(new Date().setDate(new Date().getDate()+10)).toISOString().slice(0, 10);
    const novosExpedientes = getAllIDsfromEntityBetweenDates(table, headers, today, tenDaysMore)

    try {
        const response = await enviarCorreoNovosExpedientes(novosExpedientes);
