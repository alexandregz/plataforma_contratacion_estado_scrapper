...
const { createTable, insertIntoTable, existsRecord, getTotalFilesFromExpediente, patchTable, maxDataPublicacionBD } = require('./sqliteAccions');
...
// parada por data do bucle de páxinas (SDD: page-limit-by-date)
const { localizarIndiceColunaData, normalizarDataWeb, debePararPorData } = require('./parsearDatas');
...
//
// Devolve un resumo do estado da táboa tratada: { table, encontrada, tenResultados }.
...
async function parsearResultadosLicitacionsContratos(browser, page, table, concello, entidade) {
  ...
  const resumoDoProceso = { table, encontrada: !!elementHandle, tenResultados: false };
  ...
  if (elementHandle) {
    ...
    if (headers.length > 0) {
      ...
      // Parada por data (SDD: page-limit-by-date): localizamos a columna de data e o horizonte coñecido na BD.
      // `horizonteMs` null ⇏ sen histórico ⇒ a parada por data queda desactivada (non cortar cedo nas primeiras execucións).
      const dateIdx = localizarIndiceColunaData(headers);
      let horizonteMs = null;
      if (dateIdx >= 0) {
        const nomeColunaData = String(headers[dateIdx]).replace(/ /g, "_");
        horizonteMs = maxDataPublicacionBD(table, nomeColunaData);
      }
      ...
      while (!isLastPage) {
        ...
        // Parada por data: se a última fila desta páxina xa é máis antiga ca o horizonte coñecido,
        // non pode haber nada novo máis alá (fallback conservador: só corta con datas/horizonte fiables).
        if (horizonteMs != null && pageData.length > 0) {
          const ultimaFila = pageData[pageData.length - 1];
          const ultimaMs = normalizarDataWeb(ultimaFila != null ? ultimaFila[dateIdx] : null);
          if (debePararPorData(ultimaMs, horizonteMs)) {
            console.log("   ⏹️  " + table + ": última fila máis antiga ca o horizonte, paramos por data")
            isLastPage = true;
            break;
          }
        }
