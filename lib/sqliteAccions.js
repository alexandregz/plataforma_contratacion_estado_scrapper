...
// Devolve todos os expedientes dunha táboa que AÍNDA non foron notificados (non están en email_enviados).
// Non depende da data de inserción: envía os de hoxe, onte, antonte... mentres non estean marcados.
function getExpedientesNonNotificados(table) {
    createEmailEnviados();
    // Se a táboa non existe (ex.: órganos que non publican ese tipo, como Pleno_contratos_menores),
    // non hai nada que notificar: devolvemos [] en vez de petar o correo único enteiro (SQLiteError: no such table).
    const existe = DB.query(`SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = ?1`).get(table);
    if (!existe) {
        return [];
    }
    // O primeiro campo de calquera táboa do scraper é 'Expediente' (PK).
    const rows = DB.query(`SELECT * FROM ${table} WHERE Expediente NOT IN (SELECT expediente FROM email_enviados WHERE taboa = ?1) ORDER BY fecha_insercion DESC`).all(table);
    return rows;
}
