...
// normalización de datas da web (SDD: page-limit-by-date)
const { normalizarDataWeb } = require('./parsearDatas');
...
// Máximo da data de publicación xa gardada na BD para a columna indicada (horizonte de parada por data do bucle de páxinas).
// - nomeColuna: nome de columna resolto polo header (espazos→_).
// - Devolve os ms UNIX do máximo das celas parseables, ou `null` se a táboa/columna non existe ou non hai valores parseables.
// - NON usa MAX() de SQLite: sobre texto `DD/MM/AAAA` ordenaría mal; parsea en JS (normalizarDataWeb) e compara en ms.
function maxDataPublicacionBD(table, nomeColuna) {
  try {
    const rows = DB.prepare(`SELECT ${nomeColuna} AS d FROM ${table}`).all();
    let max = null;
    for (const row of rows) {
      const ms = normalizarDataWeb(row.d);
      if (ms == null) continue;
      if (max == null || ms > max) max = ms;
    }
    return max;
  } catch (e) {
    // táboa ou columna ausente
    return null;
  }
}


module.exports = { loadDB, createTable, insertIntoTable, existsRecord, getAllIDsfromEntityBetweenDates, getTotalFilesFromExpediente, patchTable, totalRowsInTable, normalizeEuroValue, maxDataPublicacionBD };