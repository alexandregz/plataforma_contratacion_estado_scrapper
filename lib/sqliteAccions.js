...
function getAllIDsfromEntityBetweenDates(table, headers, data_comezo, data_fin) {
    const query = DB.query(`SELECT ${headers[0]} FROM ${table} WHERE date(fecha_insercion) BETWEEN date(?1) AND date(?2);`);
...
module.exports = { loadDB, createDB, insertDB, existsRecord, getAllIDsfromEntityBetweenDates };