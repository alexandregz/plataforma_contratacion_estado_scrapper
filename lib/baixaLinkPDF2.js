 ...
 */
async function baixaLinkPDF2(browser, table, el, ruta, dataPDFs, url, headersFiles) {
  ...
  // inserta en BD: [nomeFicheiro, expediente(el[0]), tipoDocumento, dataPublicacion, url]
  // adapta a orde se no teu esquema é distinta
  try {
    insertIntoTable(`${table}_files`, headersFiles, [fileName, el[0], dataPDFs?.[1] ?? '', dataPDFs?.[0] ?? '', url]);
  } catch (e) {
