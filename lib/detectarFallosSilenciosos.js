...
//
// resumos: array de { table, encontrada, tenResultados, url } producidos por parsearResultadosLicitacionsContratos
  ...
  for (const resumo of resumos) {
    if (!resumo.encontrada || !resumo.tenResultados) {
      ...
      if (total > 0) {
        ...
        const link = resumo.url ? `\n   🔗 Perfil: ${resumo.url}` : '';
        alertas.push(`${resumo.table}: ${motivo}, pero ten ${total} rexistro(s) histórico(s).${link}\n   Revisa o HTML/scraper.`);
      }
    }
  }
  return alertas;
